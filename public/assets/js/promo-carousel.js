(() => {
  ...
  function initPromoCarousel() {
    const root = document.querySelector('[data-js="promo-rail"]');
    if (!root) return;

    const track = root.querySelector('[data-js="promo-rail-track"]');
    const group = root.querySelector('[data-js="promo-rail-group"]');
    const duplicate = track?.querySelector('.c-promo-strip__group[aria-hidden="true"]');
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPromoCarousel, { once: true });
  } else {
    initPromoCarousel();
  }
})();
