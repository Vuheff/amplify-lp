...
O acesso oficial é `http://127.0.0.1:4173`. O Go Live usa a raiz `public/` e a mesma porta por configuração do workspace. A abertura direta de `public/index.html` por `file://` pode bloquear os ES Modules do deck e do modal, mas o bootstrap clássico valida as entradas do ScrollReveal e os trilhos independentes também nesse modo.
...
`check:hero` executa a validação de comportamento sem criar arquivos. Ele também roda `motion-profile-check.mjs`, que mede opacidade, distância e escala nos frames inicial, intermediário e final de `cascade`, `slide-left`, `slide-right` e `zoom`, nos perfis normal e reduzido, além da progressão sequencial das quatro barras. `check:visual` acrescenta capturas em 320, 360, 390, 430, 720 e 1440 px, além do menu mobile aberto, dos estados formulário/oferta do modal, do trilho fotográfico e do bloco de produto/valor em 390/1440 px, hover em 1440 px, drag em 430 px e páginas completas em 390/1440 px. Ambos validam a pilha de cards reais, loop em JavaScript, preço e entrega confirmados, velocidade, pausa/retomada com foco, hover, resize, visibilidade da página, ScrollReveal local, entrada única, retorno pelo histórico, link direto, falha da biblioteca, drag nos dois sentidos, gesto vertical, teclado, ARIA, controles de 44 px, overflow e fallback sem JS. O reflow equivalente a 200% também é validado em 720 px.
