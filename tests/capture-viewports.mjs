...
await send("Runtime.evaluate", {
  awaitPromise: true,
  expression: `(async () => {
    const section = document.querySelector('.c-offer');
    section.scrollIntoView({ block: 'start' });
    await new Promise((resolve) => setTimeout(resolve, 400));
  })()`,
});

const offerCardBox = await send("Runtime.evaluate", {
  returnByValue: true,
  expression: `(() => {
    const box = document.querySelector('.c-offer').getBoundingClientRect();
    return { x: box.left, y: box.top + scrollY, width: box.width, height: box.height };
  })()`,
});

const offerCardCapture = await send("Page.captureScreenshot", {
  format: "png",
  fromSurface: true,
  captureBeyondViewport: true,
  clip: { ...offerCardBox.result.value, scale: 1 },
});

await writeFile(resolve(outputDirectory, "offer-card-390.png"), Buffer.from(offerCardCapture.data, "base64"));
console.log("offer-card-390.png", offerCardBox.result.value);
...
const offerDesktopLoaded = waitForEvent("Page.loadEventFired");
await send("Page.navigate", { url: pageUrl });
await offerDesktopLoaded;
await send("Runtime.evaluate", {
  awaitPromise: true,
  expression: `(async () => {
    document.querySelector('.c-product-value').scrollIntoView({ block: 'start' });
    await new Promise((resolve) => setTimeout(resolve, 400));
  })()`,
});

const productValueDesktopBox = await send("Runtime.evaluate", {
  returnByValue: true,
  expression: `(() => {
    const box = document.querySelector('.c-product-value').getBoundingClientRect();
    return { x: box.left, y: box.top + scrollY, width: box.width, height: box.height };
...
const productValueDesktopCapture = await send("Page.captureScreenshot", {
  format: "png",
  fromSurface: true,
  captureBeyondViewport: true,
  clip: { ...productValueDesktopBox.result.value, scale: 1 },
});

await writeFile(resolve(outputDirectory, "product-value-1440.png"), Buffer.from(productValueDesktopCapture.data, "base64"));
console.log("product-value-1440.png", productValueDesktopBox.result.value);

await send("Runtime.evaluate", {
  awaitPromise: true,
  expression: `(async () => {
    document.querySelector('.c-offer').scrollIntoView({ block: 'start' });
    await new Promise((resolve) => setTimeout(resolve, 400));
  })()`,
});

const offerDesktopBox = await send("Runtime.evaluate", {
  returnByValue: true,
  expression: `(() => {
    const box = document.querySelector('.c-offer').getBoundingClientRect();
    return { x: box.left, y: box.top + scrollY, width: box.width, height: box.height };
...
const offerDesktopCapture = await send("Page.captureScreenshot", {
  format: "png",
  fromSurface: true,
  captureBeyondViewport: true,
  clip: { ...offerDesktopBox.result.value, scale: 1 },
});

await writeFile(resolve(outputDirectory, "offer-card-1440.png"), Buffer.from(offerDesktopCapture.data, "base64"));
console.log("offer-card-1440.png", offerDesktopBox.result.value);
...
const offerMobileReloaded = waitForEvent("Page.loadEventFired");
await send("Page.navigate", { url: pageUrl });
await offerMobileReloaded;
