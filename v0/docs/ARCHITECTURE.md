...
`index.html` é a composição final e mantém a ordem de leitura. Sections não serão baixadas como fragments por JavaScript, pois isso degradaria conteúdo, SEO e fallback.
...
`main.css` apenas declara `@layer` e importa os arquivos na ordem correta.
...
`assets/js/main.js` é o único entrypoint. Ele chama funções explícitas; não concentra a implementação dos módulos.
...
- sem ScrollReveal, Swiper, GSAP ou manipulação global de DOM; autoplay fica restrito aos trilhos fotográfico e de marcas isolados nos `ADR-014`/`ADR-015`/`ADR-017`.
